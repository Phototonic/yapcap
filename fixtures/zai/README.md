...
The fetcher sends `Authorization: Bearer <managed-key>` first and permits one retry with the raw
...
When both `startTime` and `endTime` provide a positive measured span, the parser uses that span
for MCP duration instead of assuming a month.
